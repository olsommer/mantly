...
`Shared business-object identifiers` and `Shared business-party references`
sections, when present, contain only references safely carried from customer
text into this isolated concern. Use those references as lookup parameters for
this concern, but do not import or perform sibling requests. Never report an
object as missing or not found unless a relevant configured lookup returned that
negative result.
