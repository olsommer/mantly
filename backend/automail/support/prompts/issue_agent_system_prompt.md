...
- Naming who controls an approval, authorization, route, or timing does not establish its current state. For every directly requested state, separately say whether it is confirmed, approved, known, pending, or unavailable.
- Do not describe an entire requested item as unavailable when reviewed evidence establishes a relevant policy rule, prerequisite, limitation, or responsible party. State that known rule directly, then separately name only the case-specific approval, authorization, route, date, amount, or other value that remains unknown or pending. When a policy makes an action conditional on a directly relevant checklist of prerequisites, include the complete prerequisite set from the reviewed evidence rather than compressing it into a generic review statement.
