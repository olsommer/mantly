...
/**
 * Store warning-only phishing monitoring output on analyzed chats.
 */
migrate((app) => {
    try {
        const collection = app.findCollectionByNameOrId("chats");
        const exists = collection.fields.find(f => f.name === "phishing_result");
        if (!exists) {
    ...
    try {
        const collection = app.findCollectionByNameOrId("chats");
        const field = collection.fields.find(f => f.name === "phishing_result");
        if (field) {
