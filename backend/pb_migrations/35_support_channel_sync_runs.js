...
/**
 * Per-channel inbound sync run history for scheduler/admin observability.
 */
...
migrate(
    (app) => {
        try {
            ...
            ensureCollection(app, {
                name: "support_channel_sync_runs",
                type: "base",
                fields: [
    ...
    (app) => {
        try {
            const col = app.findCollectionByNameOrId("support_channel_sync_runs");
            app.delete(col);
        } catch (_) {}
    },
);
