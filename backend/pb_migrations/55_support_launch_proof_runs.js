...
/**
 * Project-level launch proof run history for deploy/runtime evidence.
 */
...
migrate(
    (app) => {
        try {
            ...
            ensureCollection(app, {
                name: "support_launch_proof_runs",
                type: "base",
                fields: [
                    ...
                    { name: "ran",              type: "number",   required: false },
                    ...
                    { name: "launch_readiness", type: "json",     required: false },
                    { name: "launch_proof",     type: "json",     required: false },
                ...
                indexes: [
                    "CREATE INDEX idx_support_launch_proof_runs_project_started ON support_launch_proof_runs (project, started_at)",
                    "CREATE INDEX idx_support_launch_proof_runs_status_started ON support_launch_proof_runs (status, started_at)",
                ],
                listRule: null,
                viewRule: null,
                createRule: null,
    ...
    (app) => {
        try {
            const col = app.findCollectionByNameOrId("support_launch_proof_runs");
            app.delete(col);
        } catch (_) {}
    },
);
