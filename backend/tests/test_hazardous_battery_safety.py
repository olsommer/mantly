...
_COMPLETE_FRENCH_GUIDANCE = (
    ...
    "Ne l'expédiez pas et ne le retournez pas avant d'avoir reçu des consignes "
    "confirmées pour les marchandises dangereuses. En cas de fumée, d'incendie ou "
...
_COMPLETE_GERMAN_GUIDANCE = (
    ...
    "oder zurücksenden, bevor Sie bestätigte Gefahrgut-Anweisungen erhalten. Wenn "
...
_COMPLETE_SPANISH_GUIDANCE = (
    ...
    "seguro. No lo envíe ni lo devuelva antes de recibir instrucciones confirmadas "
    "de seguridad para mercancías peligrosas. Si hay humo, fuego o calor inusual, "
...
_COMPLETE_ITALIAN_GUIDANCE = (
    ...
    "solo se è sicuro. Non lo spedisca né lo restituisca prima di ricevere istruzioni "
    "confermate di sicurezza per merci pericolose. In caso di fumo, fuoco o calore "
...
def test_damaged_lithium_battery_policy_detection(text: str) -> None:
    ...
    assert assessment.prompt_context()["obligation"]["id"] == (
        LITHIUM_BATTERY_SAFETY_OBLIGATION_ID
    )
    ...
    assessment = assess_lithium_battery_safety(
        messages=[
            ...
            },
            {
                "direction": "customer",
                "body": (
                    "The battery is no longer leaking or hot and the issue is resolved."
                ),
            },
        ],
    )
...
def test_resolved_leak_does_not_hide_a_separate_current_heat_hazard() -> None:
    assessment = assess_lithium_battery_safety(
        body="The battery leak has stopped, but it is still unusually hot."
    )
...
def test_not_only_hot_but_smoking_is_an_active_battery_hazard() -> None:
    assessment = assess_lithium_battery_safety(
        body="The lithium battery is not only hot, but smoking."
    )
...
def test_safety_guidance_checker_requires_every_immediate_policy_element() -> None:
    ...
    missing = missing_lithium_battery_safety_guidance(
        "Please isolate the leaking battery and wait for our team."
    )
...
def test_contradictory_positive_guidance_fails_closed(
    ...
    missing = missing_lithium_battery_safety_guidance(
        _COMPLETE_GUIDANCE + " " + contradiction
    )
...
def test_ordinary_cold_storage_and_prohibitions_do_not_trigger_unsafe_placement(
    ...
    assert missing_lithium_battery_safety_guidance(
        _COMPLETE_GUIDANCE + " " + ordinary_storage_guidance
    ) == ()
...
def test_unsafe_multilingual_imperatives_fail_closed(
    ...
    missing = missing_lithium_battery_safety_guidance(
        _COMPLETE_GUIDANCE + " " + unsafe_imperative
    )
...
def test_jurisdiction_specific_emergency_number_variants_fail_closed(
    ...
    missing = missing_lithium_battery_safety_guidance(
        _COMPLETE_GUIDANCE + " " + number_guidance
    )
...
def test_plain_order_or_reference_numbers_are_not_emergency_guidance(
    ...
    missing = missing_lithium_battery_safety_guidance(
        _COMPLETE_GUIDANCE + " " + ordinary_reference
    )
...
def test_reply_safety_boundary_uses_latest_customer_message_and_clear_reason() -> None:
    ...
    assert lithium_battery_reply_safety_blocked_reason(
        subject="Leaking lithium battery",
        messages=_HAZARD_MESSAGES,
    ...
    class FakeAutomationAgent:
        def invoke(
            ...
            *,
            ...
            answer = (
                "Please isolate the parcel and wait for our team."
                if len(prompts) == 1
                else _COMPLETE_GUIDANCE
            )
            return {
...
def test_grounding_accepts_system_safety_policy_with_zero_knowledge_articles(
    ...
    assert json.loads(system_policy_json)["evidenceId"] == (
        LITHIUM_BATTERY_SAFETY_POLICY_ID
    )
